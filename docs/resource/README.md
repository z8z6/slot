...
内建资源 ID 位于 `include/Resource/BuiltinResource.h`。Mesh 和 Material 在 `ResourceManager::RegisterBuiltinResources` 显式注册；普通 ShaderProgram 由 `shader/*.shader.json` 描述，CMake 调用 `scripts/generate_shader_registry.py` 生成显式注册代码。文件级静态 Registry 已移除。
