...
场景数据使用 `ResourceRef<T>` 保存规范 Asset ID；GameObject 绑定 Mesh 和 Material，Material 再引用 ShaderProgram。`DX12RenderBatch::Init` 将这条依赖链解析为带 Index/Generation 的 `ResourceHandle<T>`。渲染热路径只使用 Handle，避免逐帧字符串查询，Generation 也为后续卸载和热重载提供旧句柄检测边界。
